...
class YellowPage(AgentManager, object):
    ...
    def start_monitoring(self):
        def monitor():
            ...
            while True:
                unavailable_agents = self.verify_agents_availability()
                current_agent_ids = set(get_all_agents_registered().keys())
                # Se comprueba los id de los agentes que no estan disponibles en el listado de agentes
                for id_agent in unavailable_agents:
                    if id_agent in current_agent_ids:
                        # Se elimina el agente de la lista de agentes, pero se mantiene en la lista de agentes inactivos
                        # pasando active a False y se guarda en el archivo de datos
                        self.agents[id_agent]['active'] = False
                        register_agents(self.agents)
                # Se envian solo los agentes que no estan en la lista de agentes inactivos
                agents_active = {id_agent: agent for id_agent, agent in self.agents.items() if agent.get('active', True)}
                send_list_agents(agents_active, self.keys_asimetrics, self.nameserver, self.management_logs)
                # Esperar 90 segundos antes de la siguiente verificación
                time.sleep(15)
    ...
    def verify_agents_availability(self):
        ...
        for id_agent, agent_info in list(self.agents.items()):
            try:
                ...
                agent_proxy.ping()
                # Si el agente responde, asegúrate de que esté marcado como activo
                self.agents[id_agent]['active'] = True
            except Exception as e:
        ...
        # Ya no eliminamos agentes inactivos, solo los marcamos como inactivos
